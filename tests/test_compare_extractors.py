...
class TestCompareUrl:
    ...
    def test_writes_images_section_when_present(self, tmp_path):
        """Images are downloaded next to the .md file (out_dir/images/) and
        embedded via local path, same convention as the real resource store —
        not just listed as remote URLs."""
        out_dir = tmp_path / "cmp"
        tavily_fn = lambda url: (  # noqa: E731
            "Title",
            "body",
        ...
        with (
            ...
            patch(
                "linkedin_api.compare_extractors.download_image_to_store",
                return_value="images/fake-hash.jpg",
            ) as mock_download,
            _mock_backends(httpx_fn, tavily_fn),
        ):
            compare_url("https://example.com/article", out_dir=out_dir)
        ...
        mock_download.assert_called_once_with(
            "https://cdn.example.com/img.jpg", base_dir=out_dir
        )
        tavily_file = next(out_dir.glob("*-tavily.md"))
        httpx_file = next(out_dir.glob("*-httpx.md"))
        assert "![](images/fake-hash.jpg)" in tavily_file.read_text()
        assert "https://cdn.example.com/img.jpg" not in tavily_file.read_text()
        assert "## Images" not in httpx_file.read_text()

    def test_image_download_failure_omits_images_section(self, tmp_path):
        out_dir = tmp_path / "cmp"
        tavily_fn = lambda url: (  # noqa: E731
            "Title",
            "body",
        ...
        tavily_file = next(out_dir.glob("*-tavily.md"))
        assert "## Images" not in tavily_file.read_text()
